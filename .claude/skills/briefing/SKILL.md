...
Delegate research to 4 Sonnet agents for cost efficiency. Use the Bash tool to run each agent as a separate `claude -p` process. Launch all 4 Bash calls IN PARALLEL in a single message.
...
Template:
```
cat <<'RESEARCH_PROMPT' | claude -p --model claude-sonnet-4-6 --allowedTools "WebSearch,WebFetch,Bash" > /tmp/fo-briefing-{name}.md 2>/dev/null
