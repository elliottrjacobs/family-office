...
These instructions apply to every agent (every `.Codex/skills/*/SKILL.md`). They OVERRIDE default behavior and must be followed exactly.
...
```
.Codex/skills/ → SKILL.md files defining each agent
.Codex/agents/ → research sub-agent definitions
.Codex/settings.json → permission allow-list
