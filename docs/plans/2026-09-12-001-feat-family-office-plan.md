...
- **Stop conditions:** never read, copy, or commit anything from the real office (a private legacy vault) except through `fo migrate-v1` when the user runs it; never add a code path that writes to a brokerage, bank, or card provider; never track a path under `office/` in this repo; stop and ask if a phase's acceptance check cannot be met without changing a Key Technical Decision.
- **Execution profile:** clean rebuild in the existing `family-office` repository. The original implementation was retired; it is not a dependency of this package. Build phases A through F on `codex/family-office`, with verified commits per unit and phase checkpoints in CI. The default branch retains the original implementation until the replacement is ready; no working hybrid or backward compatibility is required. Carry forward only verified behavior, rubrics, and migration knowledge from the archive. Preserve planning and reference documents. The product, repository, and package remain named `family-office`; `fo` remains the CLI. Version numbers are release metadata, not part of the product name. `session-settled: user-directed` (clean rebuild and unchanged product name; rejected alternative: incremental replacement with a working hybrid).
- **Tail ownership:** the implementing agent owns tests, CI, docs, connection setup, account discovery, profile configuration, and live diagnostics. The user supplies only credentials/browser consent when required, chooses the office git remote, and owns the v1 cutover decision. Pending live access does not block implementation against synthetic fixtures; live validation and reconciliation remain required before cutover. Do not hand the user manual JSON edits, account-ID lookups, or command checklists. `session-settled: user-directed` (2026-09-13: automate setup and verification; rejected alternative: a user-operated smoke-test checklist).
...
The migration design supports a legacy Obsidian vault that is not a Git repository. A Node webapp under launchd reads nine v1 profile files there; the user has said it does not need to be carried forward. The draft v2 plan assumes a gitignored SQLite store, which would hide household data from Codex cloud and from any second machine.
...
- A1. A user operating a private office on a designated writer machine, with optional additional local or cloud sessions.
...
| 2. Codex parity now or later | Now, but by design (canonical `AGENTS.md`, one skill copy, generated agents), not by generation from Claude files | the user uses both hosts; v1's generated mirror could not run |
...
- The office has no git remote until the user decides D1; the design must not depend on one, and the deliverables that need one (R23, F3, KTD11, the cloud parts of U14) are conditional on it.
...
- the user is the sole operator; no shared-machine or multi-user permission model is needed.
...
- Codex applies a trusted project's `.codex/config.toml` over the global config, so the office's `workspace-write` sandbox overrides global sandbox settings; if a Codex release changes that layering, the office README note in U3 must change.
...
- **Approach:**
  ...
  5. Remove tracked legacy implementation trees from this feature branch, preserving the license, planning documents, and historical explainer under `docs/`. Do not touch ignored private files or the real office. U2 and U11 create new instruction and skill files; later references to deleting legacy files are satisfied by this clean-start removal.
...
- **Execution note:** record real provider responses once by hand into fixtures (scrubbed), then develop against fixtures; the live path is smoke-tested manually by the user.
...
- **Patterns to follow:** the legacy launchd plist shape (working directory, log paths) for the plist template.
- **Test scenarios:**
...
- **Execution note:** build against the fake v1 vault only; the user runs it once against the real vault with `--dry-run` first.
...
- **Verification:** `fo migrate-v1 --dry-run` on the real vault (the user) reports zero unmapped accounts before a real run.
...
| Agent-run live check | `fo setup` discovers accounts, authenticates, syncs, and runs diagnostics on the Mini; agent validates provider outputs | providers work end to end | required before cutover; agent owns execution, the user supplies credential entry/consent only; pending access does not block fixture-based implementation |
...
- Skills can be shared across hosts with per-entry symlinks (`~/.agents/skills/<name>` and `~/.claude/skills/<name>` both point into one source directory), which is the mechanism U3 reuses.
